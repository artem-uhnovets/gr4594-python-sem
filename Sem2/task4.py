...
Пользователь вводит
одно число N – количество арбузов.
...
number = int(input('Enter a number of watermelons: '))
wMax = int(input(f'Enter a weight of 1 watermelon: '))
wMin = wMax
for i in range(number-1):
    weight = int(input(f'Enter a weight of {i + 2} watermelon: '))
    if weight > wMax:
        wMax = weight
    elif weight < wMin:
        wMin = weight
